...
const Cards = ({ title, alpha3Code, imageSrc,population,name,region,capital }) => {
 ...
 const BlackColor="#202631"
  ...
  return(<>
  <Box maxW={{sm:"xlg",lg:'sm'}} borderWidth='1px' borderRadius='lg' overflow='hidden'mt={5} maxH={{sm:"470",md:"750px",lg:"370px"}}   key={name}  >
    <Link to={`/${alpha3Code}`}>
     <Box >
      ...
      <Box p='6' bgColor={`${colorValue?`${ WhiteColor}`:`${BlackColor}`} `}  h={{sm:"200px",md:"200px",lg:"150px"}}  color={`${colorValue?`${BlackColor}`:`${ WhiteColor}`} `}>
